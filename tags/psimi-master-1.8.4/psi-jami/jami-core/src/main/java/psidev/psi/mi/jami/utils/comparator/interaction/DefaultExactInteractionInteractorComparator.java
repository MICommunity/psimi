...
import psidev.psi.mi.jami.utils.comparator.participant.DefaultExactParticipantInteractorComparator;
...
public class DefaultExactInteractionInteractorComparator {
    ...
    private static boolean compareParticipants(Collection<? extends Participant> participants1, Collection<? extends Participant> participants2) {
        ...
        while (f1Iterator.hasNext()){
            ...
            for (Participant f2 : f2List){
                if (DefaultExactParticipantInteractorComparator.areEquals(f1, f2)){
                    f2ToRemove = f2;
                    break;
                }
            }
