...
/**
 * Unit tester for SpokeExpansion
 *
...
public class SpokeExpansionTest {

    private SpokeExpansion expansion = new SpokeExpansion();

    @Test
    public void test_method(){

        Assert.assertEquals(CvTermUtils.createMICvTerm(ComplexExpansionMethod.SPOKE_EXPANSION, ComplexExpansionMethod.SPOKE_EXPANSION_MI), expansion.getMethod());
    }
    ...
    public void test_expand_default_interaction(){
        ...
        for (BinaryInteraction binaryInteraction : nary_expanded){
            ...
            Assert.assertTrue(binaryInteraction.getParticipantB() == p2 || binaryInteraction.getParticipantB() == p3);
            Assert.assertTrue(binaryInteraction.getParticipantA() == p1);
        }
    }
