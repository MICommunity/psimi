...
import psidev.psi.mi.jami.tab.io.writer.Mitab25BinaryWriter;
...
public class Mitab25BinaryWriterTest {
    ...
    public void test_write_binary_list() throws DataSourceWriterException, ParseException {
        ...
        ParticipantEvidence participantA2 = new MitabParticipantEvidence(new MitabProtein("protein1", "full name protein1"));
        // add identifiers
        participantA2.getInteractor().getIdentifiers().add(XrefUtils.createUniprotIdentity("P12345"));
        participantA2.getInteractor().getIdentifiers().add(XrefUtils.createUniprotSecondary("P12346"));
        participantA2.getInteractor().getIdentifiers().add(XrefUtils.createXref("intact", "EBI-12345"));
        // add aliases
        participantA2.getInteractor().getAliases().add(AliasUtils.createGeneName("brca2"));
        participantA2.getInteractor().getAliases().add(AliasUtils.createGeneNameSynonym("brca2 synonym"));
        participantA2.getAliases().add(AliasUtils.createAuthorAssignedName("\"bla\" author assigned name"));
        // species
        participantA2.getInteractor().setOrganism(new MitabOrganism(9606, "human", "Homo Sapiens"));
        //participantA.getAliases()
        ParticipantEvidence participantB2 = new MitabParticipantEvidence(new MitabProtein("protein2", "full name protein2"));
        // add identifiers
        participantB2.getInteractor().getIdentifiers().add(XrefUtils.createUniprotIdentity("P12347"));
        participantB2.getInteractor().getIdentifiers().add(XrefUtils.createUniprotSecondary("P12348"));
        participantB2.getInteractor().getIdentifiers().add(XrefUtils.createXref("intact", "EBI-12346"));
        // species
        participantB2.getInteractor().setOrganism(new MitabOrganism(9606, "human", "Homo Sapiens"));

        BinaryInteractionEvidence binary2 = new MitabBinaryInteractionEvidence(participantA2, participantB2);
        participantA2.setInteractionEvidence(binary2);
        participantB2.setInteractionEvidence(binary2);

        // detection method
        binary2.setExperiment(new MitabExperiment(new MitabPublication()));
        binary2.getExperiment().setInteractionDetectionMethod(new MitabCvTerm("pull down", "MI:xxx2"));
        // first author
        binary2.getExperiment().getPublication().setPublicationDate(MitabUtils.PUBLICATION_YEAR_FORMAT.parse("2006"));
        binary2.getExperiment().getPublication().getAuthors().add("author1");
        binary2.getExperiment().getPublication().getAuthors().add("author2");
        // publication identifiers
        binary2.getExperiment().getPublication().setPubmedId("12345");
        binary2.getExperiment().getPublication().assignImexId("IM-1");
        // interaction type
        binary2.setInteractionType(CvTermUtils.createMICvTerm("association", "MI:xxxx"));
        // source database
        MitabSource source2 = new MitabSource("intact");
        source2.setMIIdentifier("MI:xxx1");
        binary2.getExperiment().getPublication().setSource(source2);
        // interaction identifiers
        binary2.getIdentifiers().add(XrefUtils.createIdentityXref("intact", "EBI-xxxx"));
        binary2.getIdentifiers().add(XrefUtils.createXrefWithQualifier("imex", "IM-1-1", "imex-primary"));
        // confidences
        binary2.getConfidences().add(new MitabConfidence("author-score", "high", null));
        ...
        String expected_line = "uniprotkb:P12345" +
                ...
                "\tintact:EBI-xxxx" +
                "\tauthor-score:high";
        String expected_line2 = "uniprotkb:P12345" +
                "\tuniprotkb:P12347" +
                "\tuniprotkb:P12346|intact:EBI-12345" +
                "\tuniprotkb:P12348|intact:EBI-12346" +
                "\tpsi-mi:protein1(display_short)|psi-mi:full name protein1(display_long)|uniprotkb:brca2(gene name)|uniprotkb:brca2 synonym(gene name synonym)|intact:\\\"bla\\\" author assigned name(author assigned name)" +
