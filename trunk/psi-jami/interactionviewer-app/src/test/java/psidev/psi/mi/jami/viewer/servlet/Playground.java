...
public class Playground {
    ...
    public void test_play_psicquic() throws IOException {
        ...
        try {
            ...
            switch (fileType){
                case mitab:
                    miDataSource = miFactory.getInteractionSourceWith(optionFactory.getMitabOptions(InteractionCategory.evidence, ComplexType.binary,
                            true, null, dataStream));
                    interactionWriter = new MIJsonBinaryEvidenceWriter(writer,this.fetcher);
                    break;
                case psimi_xml:
                    miDataSource = miFactory.getInteractionSourceWith(optionFactory.getXml25Options(InteractionCategory.evidence, ComplexType.n_ary,
                            true, null, dataStream, null, new InMemoryPsiXmlCache()));
