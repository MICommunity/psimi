...
//            }
                ...
                if (allRefs != null) {
                    for (DbReference ref : allRefs) {
                        if (ref.hasRefType() && !ref.getRefType().equalsIgnoreCase("identity")) {
                            CrossReference cr = buildCrossReference(ref);
...
//        }
        ...
        if (xmlInteractor.getNames() != null) {
            ...
            if (shortLabel != null) {
                psidev.psi.mi.tab.model.Alias a = new psidev.psi.mi.tab.model.AliasImpl(UNKNWON, shortLabel);
                a.setAliasType("display_short");
//            a.setAliasType(SHORT_LABEL);
            ...
            if (fullName != null) {
                psidev.psi.mi.tab.model.Alias a = new psidev.psi.mi.tab.model.AliasImpl(UNKNWON, fullName);
                a.setAliasType("display_long");
                tabAliases.add(a);
            }
        ...
        if (xmlInteractor.hasOrganism()) {
            ...
            Collection<CrossReference> organismXrefs = new ArrayList<CrossReference>();
            ...
            if (o.hasNames()) {
                ...
                if (shortLabel != null) {
                    organismXrefs.add(CrossReferenceFactory.getInstance().build("taxid", taxId, shortLabel));
                }
                ...
                if (shortLabel == null && fullName == null) {
                    organismXrefs.add(CrossReferenceFactory.getInstance().build("taxid", taxId));
                }
            } else {
                organismXrefs.add(CrossReferenceFactory.getInstance().build("taxid", taxId));
            }

            OrganismFactory.getInstance().build(organismXrefs);
        ...
        if (xmlParticipant.hasBiologicalRole()) {
            ...
            Collection<CrossReference> tabBioRole = new ArrayList<CrossReference>();
        ...
        if (xmlParticipant.hasExperimentalRoles()) {
            ...
            Collection<CrossReference> tabExpRoles = new ArrayList<CrossReference>();
        ...
        if (xmlInteractor.getInteractorType() != null) {
            ...
            Collection<CrossReference> tabInteractorType = new ArrayList<CrossReference>();
        ...
        if (xmlParticipant.getFeatures() != null) {
            ...
            Collection<psidev.psi.mi.tab.model.Feature> tabFeatures
        ...
        if (xmlParticipant.getAttributes() != null) {
            ...
            Collection<Integer> tabStoichiometry
        ...
        if (xmlParticipant.getParticipantIdentificationMethods() != null) {
            ...
            Collection<psidev.psi.mi.tab.model.ParticipantIdentificationMethod> tabPartIdentMethods
                    = new ArrayList<psidev.psi.mi.tab.model.ParticipantIdentificationMethod>();
            ...
            for (ParticipantIdentificationMethod xmlPartIdentMethodsRole : xmlPartIdentMethodsRoles) {
                psidev.psi.mi.tab.model.ParticipantIdentificationMethod cr
                        = CvConverter.toMitab(xmlPartIdentMethodsRole, ParticipantIdentificationMethodImpl.class);
    ...
    public psidev.psi.mi.xml.model.Interactor fromMitab(psidev.psi.mi.tab.model.Interactor tabInteractor) throws XmlConversionException {
        ...
        //Fields 23 24
        //TODO Noe is correct in this place?
        ...
        if (tabInteractor.getInteractorTypes() != null && !tabInteractor.getInteractorTypes().isEmpty()) {
            ...
            if (interactorType.getXref() != null && interactorType.getXref().getPrimaryRef() != null) {
                String id = interactorType.getXref().getPrimaryRef().getId();
                //TODO Add more types of interactors
                ...
                // we do not have a psi-mi CVTerm, so the type is unknown
                interactorTypeName.setShortLabel("unknown participant");
                interactorTypeName.setFullName("unknown participant");
