 ...
 * @see psidev.psi.mi.xml254.jaxb.OpenCvType
...
public class OpenCvTypeConverter {
     ...
     *          psidev.psi.mi.xml254.jaxb.OpenCvType jUnit = confidence.getUnitName();
     ...
     */
    public <CV extends psidev.psi.mi.xml.model.OpenCvType> CV fromJaxb( psidev.psi.mi.xml254.jaxb.OpenCvType jOpenCvType,
                                                                        Class<CV> clazz )
            throws ConverterException {
        ...
        if ( jOpenCvType.getAttributeList() != null ) {
            for ( psidev.psi.mi.xml254.jaxb.Attribute jAttribute : jOpenCvType.getAttributeList().getAttributes() ) {
     ...
     * @return
     */
    public psidev.psi.mi.xml254.jaxb.OpenCvType toJaxb( psidev.psi.mi.xml.model.OpenCvType mOpenCvType ) {

        if ( mOpenCvType == null ) {
            throw new IllegalArgumentException( "You must give a non null JAXB OpenCvType." );
        }

        psidev.psi.mi.xml254.jaxb.OpenCvType jOpenCvType = new psidev.psi.mi.xml254.jaxb.OpenCvType();
        ...
        if ( mOpenCvType.hasAttributes() ) {
            ...
            if ( jOpenCvType.getAttributeList() == null ) {
                jOpenCvType.setAttributeList( new AttributeList() );
            }
